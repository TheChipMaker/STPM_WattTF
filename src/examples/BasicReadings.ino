...
void setup()
{
  ...
  vcfg.seriesLine = 3 * 133000.0f;    // 399 k modify based on your hardware configuration
  vcfg.seriesNeutral = 3 * 133000.0f; // 399 k modify based on your hardware configuration
  vcfg.senseR2 = 470.0f;
  ...
  Serial.print("Voltage LSB: ");
  Serial.println(meter.voltageLSB(), 10);
  Serial.print("Current LSB: ");
  Serial.println(meter.currentLSB(), 10);
}
...
void loop()
{
  ...
  meter.updateEnergy(); // sample + accumulate (call every loop)
